...
class Solver:
    ...
    def __init__(self, name, model, residual_loader: ResidualLoader, optimizer, lossfunc) -> None:
        self.name = name
        self.model = model
...
def _printProgress(epoch, epochs, log_epoch, prefix = '', suffix = '', length = 20, fill = '#', printEnd = '\r'):
    filledLength = int(length * (epoch % log_epoch) // log_epoch)
    if epoch % log_epoch == 0: 
        filledLength = length
        printEnd = '\n'
    bar = fill * filledLength + '.' * (length - filledLength)
    print(f'{prefix} [{bar}] [{epoch:>5d}/{epochs:>5d}]% {suffix}', flush=True, end = printEnd)
