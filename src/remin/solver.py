    ...
    def compile(self, optimizer, lossfunc, backend='inductor', fullgraph=False):
        ...
        def _residual_loss(params, xs):
            ...
            for i in range(self.numres):
                domain = xs[i]
                resloss += lossfunc(self.equations[i](params, *domain), torch.zeros_like(domain[0]))
            return resloss
        
        residual_loss = torch.compile(traceable(_residual_loss), backend=backend, fullgraph=fullgraph)
