...
class Cut(Geometry):
    ...
    def _generate(self, time = None) -> None:
        self.lhs._generate(time)
        cond = np.logical_not(
            self.multiple_and(self.lhs.cpoints, self.rhs._rule())
            )
    ...
    def _rule(self):
        def rule(cpoints):
            ...
            for i, dim in enumerate(self.dimensions):
                cond = np.logical_and(cond, 
                       (cpoints[:,i] >= dim[0]) & (cpoints[:,i] <= dim[1])
                    )
            return cond
        return [rule]
...
class Disk(Geometry):
    ...
    def _generate(self, time = None) -> None:
        if time is not None:      
            ...
            radius, theta = np.sqrt(coll[:,1:2]) * self.radius, 2 * np.pi * coll[:,2:3]
            self.time = Time(time.t0, time.t1)
            self.time.cpoints = coll[:,0:1] * (time.t1 - time.t0) + time.t0
        else:
            coll = lhs(2, self.n_coll)
            radius, theta = np.sqrt(coll[:,0:1]) * self.radius, 2 * np.pi * coll[:,1:2]
...
class Ring(Geometry):
    ...
    def _generate(self, time: Time = None) -> None:
        if time is None:
            ...
            self.cpoints = self.radius * np.hstack((np.cos(theta), np.sin(theta))) + self.center
        else:
            coll = lhs(2, self.n_coll)
    ...
    def _rule(self):
        def rule(cpoints):
            return np.allclose(
                ((cpoints - self.center)**2).sum(axis=1), self.radius**2
            )
        return [rule]
